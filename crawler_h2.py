...
from scripts import buyerHareruya2
from scripts import supabaseUtil

# Hareruya2クローラー
class hareruya2Crawler():
    def exec(self,wrapper,supabase):
        # 市場情報保存用のディレクトリを用意する
        market_dir = './data/market/hareruya2'
    ...
    def _getPageList(self):
        listItem = [
            {'name': 'スカーレット&バイオレット', 'num': 55},
            {'name': 'ソード＆シールド', 'num': 17}
        ]
        return listItem
    ...
    def _crawl(self, wrapper, market_dir:str):
        hareruya2Bot = buyerHareruya2.buyerHareruya2Bot()
        listItem = self._getPageList()
        
        for item in listItem:
            print(item['name'])
            hareruya2Bot.download(wrapper,item['num'],market_dir)
    ...
    def _upload(self, supabase, card_dir:str, market_dir:str):
        ...
        loader = buyerHareruya2.hareruya2BuyCsvLoader()
        ...
        for i in range(0, len(records), 100):
            ...
            converted = editor.getShopBuyupDaily4Hareruya2(batch, cardDf)
...
hareruya2 = hareruya2Crawler()
hareruya2.exec(wrapper, supabase)
