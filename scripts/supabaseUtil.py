...
class batchEditor:
 ...
 # shop_buyup_daily 用の情報を生成する
    def getShopBuyupDaily4Fullahead(self, records, cardDf):
        items = []
        if len(records) <= 0:
            return items
        timestamp = datetime.datetime.utcnow()
        for item in records:
            df = cardDf[cardDf['master_id'].str.contains(item['expansion']+'_'+item['cn']+'_')]
            if len(df) == 0:
                continue
 ...
 # shop_buyup_daily 用の情報を生成する
    def getShopBuyupDaily4Cardrush(self, records, cardDf):
        items = []
        if len(records) <= 0:
            return items
        timestamp = datetime.datetime.utcnow()
        for item in records:
            df = cardDf[cardDf['name'] == item['name']]
            if len(df) == 0:
                continue
            df = df[df['cn'] == item['cn']]
            if len(df) == 0:
                continue
            df = df[df['sub_type'] == item['type']]
            if len(df) == 0:
                continue
            if int(item['mirror']) == 1:
                df = df[df['is_mirror'] == True]
